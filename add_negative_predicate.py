...
# txt version
def generate_negative_predicate(kg_file, types_file, output_file, predicate, n):
			...
			# change o
				...
				if len(possible_o_ids) != 0:
					...
					while not found and len(possible_o_ids) != 0:
						...
						already_connected = in_kg(s, predicate, rand_node, triples)  
						if not already_connected:
...
def add_negative_predicates_to_neo4j(file_name, sample=None):
	negatives = read_facts(file_name)
	if sample is None:
		query = "MATCH (s), (o) WHERE id(s)={} AND id(o)={} CREATE (s)-[r:`{}`]->(o) RETURN r"
	else:
		query = "MATCH (s), (o) WHERE id(s)={} AND id(o)={} CREATE (s)-[r:`{}`]->(o) SET r.sample='{}' RETURN r"

	for s, p, o in negatives:
		if sample is None:
			result = list(GRAPH.run(query.format(s, o, p)))
		else:
			result = list(GRAPH.run(query.format(s, o, p, sample)))
		if len(result) < 1:
...
if __name__ == '__main__':
	...
	add_negative_predicates_to_neo4j('drkg_sample/generated_negatives_drkg.txt')
